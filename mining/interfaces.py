...
class ShareManagerInterface(object):
    ...
    def on_submit_share(self, worker_name, block_hash, difficulty, timestamp, is_valid, ip, invalid_reason, share_diff, job_id):
        log.info("%s [%s] diff(%f/%f) job_id(%s) share(%i) %s %s" % (worker_name, ip, share_diff, difficulty, job_id, difficulty, 'valid' if is_valid else 'INVALID', invalid_reason))
