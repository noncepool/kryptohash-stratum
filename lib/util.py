...
def reverse_hash_80(h):
    # This only revert byte order, nothing more
    if len(h) != 80:
        raise Exception('hash must have 80 hexa chars')
    
    return ''.join([ h[72-i:80-i] for i in range(0, 80, 8) ])
...
def address_to_pubkeyhash(addr):
    ...
    ver = addr[0]
    cksumA = addr[-4:]
    cksumB = kshake320_hash.getHash256((addr[:-4]))[:4]
